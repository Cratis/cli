...
1. **Install the CLI.**
   ...
   Download the pre-built native binary from the [latest release](https://github.com/Cratis/cli/releases/latest) and put it on your `PATH`. Release assets are named with their version, so resolve the version first:

   ```bash title="Install the native binary"
   # x64 (Intel/AMD) — swap x64 for arm64 throughout for an ARM machine
...
If `cratis run` has already pulled a Stage sandbox image, `cratis update` checks that too, and pulls a newer one when it's out — the same check every ordinary command already runs quietly in the background, so a Stage update never comes as a surprise. If you've never run `cratis run`, there's no image on your machine, and `cratis update` says nothing about it — you didn't opt into a multi-hundred-megabyte download by updating the CLI.
