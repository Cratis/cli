...
`PATH` is a Screenplay (`.play`) file, or a folder. It defaults to the current directory. A folder compiles every `.play` file beneath it **together as one application**, including nested folders. Concepts and events declared in one file can be referenced from another; you do not need to repeat their declarations. Keep unrelated applications in separate root folders, or target each application file individually. The summary's `Files` count is the number of discovered `.play` files, not the number of applications.
