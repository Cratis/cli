...
/// </summary>
[LlmDescription("Generates a CHRONICLE.md documentation file and configures AI tools (Claude Code, GitHub Copilot, Cursor, Windsurf, Pi) for the current project. Run once per project, and again with --refresh after upgrading the CLI.")]
...
public class InitCommand : AsyncCommand<InitSettings>
{
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, InitSettings settings, CancellationToken cancellationToken)
    {
        ...
        {
            // Skipping an existing file is right - it may have been edited - but staying silent about a
            // catalog generated by an older CLI is not: it still describes that CLI's commands, and an
            // agent reading it has no way to know. Say so, and name the command that fixes it.
            var existing = await File.ReadAllTextAsync(chronicleMdPath, cancellationToken);
            var generatedWith = GeneratedContextVersion.ReadFrom(existing);

            allActions.Add(GeneratedContextVersion.IsStale(generatedWith, RunningVersion)
                ? $"CHRONICLE.md was generated by CLI v{generatedWith} but this is v{RunningVersion} - run 'cratis init --refresh' to update the command catalog"
                : "CHRONICLE.md already exists (skipped, use --force to overwrite)");
        ...
        if (settings.Tool is not null)
        {
            ...
            {
                OutputFormatter.WriteError(format, $"Unknown AI tool: '{settings.Tool}'", "Valid tools: claude, copilot, cursor, windsurf, pi", ExitCodes.ValidationErrorCode);
