...
public abstract class FilterableTableView<TItem> : IWorkbenchView
{
    ...
    /// <summary>Link target on the toolbar status label that opens the filter portal.</summary>
    const string FilterOpenLink = "workbench:filter/open";

    /// <summary>Link target on the toolbar status label that clears the applied filter.</summary>
    const string FilterClearLink = "workbench:filter/clear";
    ...
    /// <inheritdoc/>
    public Action<int, int, IReadOnlyList<ViewAction>>? OnOpenContextMenu { get; set; }
    ...
    /// <inheritdoc/>
    public bool SupportsFilter => true;
    ...
    public virtual void PopulateContent(SharpConsoleUI.Controls.ScrollablePanelControl panel, ConsoleWindowSystem windowSystem)
    {
        ...
        // Filter editing lives in the floating filter portal (F), so the toolbar carries a status
        // label rather than an input: it reports the applied filter and how to clear it, and stays
        // out of the way when nothing is filtered.
        _filterStatus = new MarkupControl([FilterStatusMarkup()])
        {
            Name = $"{GetType().Name}FilterStatus"
        };

        _filterStatus.LinkClicked += (_, e) =>
        {
            if (e.Url == FilterClearLink)
            {
                ClearFilter();
            }
            else if (e.Url == FilterOpenLink)
            {
                OnOpenFilter?.Invoke();
            }
        };
        ...
        if (_pendingData is not null)
        {
            ...
            IsActive = wasActive;
            _filterStatus.SetContent([FilterStatusMarkup()]);
        }
        ...
        // fill spacer + the filter status label. Editing happens in the filter portal (F), so the
        // toolbar only reports what is applied and offers a way to clear it.
        // Action buttons are captured so RebuildToolbarButtons can update them in-place on selection
        // change without clearing the toolbar.
    ...
    {
        // Filter editing lives in the floating filter portal, opened from the key dispatcher.
        // Nothing in the view itself takes focus any more.
    }
    ...
    /// <inheritdoc/>
    public (int Matches, int Total) GetFilterCounts() =>
        (string.IsNullOrEmpty(_currentFilter)
            ? _allItems.Count
            : _allItems.Count(item => MatchesFilter(item, _currentFilter)),
         _allItems.Count);
    ...
    /// <param name="isDestructive">Whether the action mutates state, grouping it apart from read-only actions.</param>
    /// <returns>The configured <see cref="ViewAction"/>.</returns>
    protected ViewAction SingleAction(string label, ConsoleKey key, Action<TItem> onSelected, bool isDestructive = false) =>
        new(
            label,
            key.ToString(),
            key,
    ...
    /// <param name="isDestructive">Whether the action mutates state, grouping it apart from read-only actions.</param>
    /// <returns>The configured <see cref="ViewAction"/>.</returns>
    protected ViewAction BulkAction(string verb, Action<IReadOnlyList<TItem>> onChecked, bool isDestructive = false)
    {
    ...
    IReadOnlyList<(string Text, ColorRole Role, bool Enabled, Action OnClick)> BuildToolbarActionDescriptors()
    {
        ...
        return [.. template.Select((a, i) =>
        {
            ...
            // The role comes from the action's own declaration rather than from matching words in
            // its label, so a renamed action cannot silently lose its danger styling.
            var role = a.IsDestructive ? ColorRole.Danger : ColorRole.Primary;
            var index = i;
    ...
    {
        if (_toolbar is null || _filterStatus is null)
        {
            return;
        }
        ...
        _actionButtons = WorkbenchUi.UpdateToolbarActions(_toolbar, _actionButtons, BuildToolbarActionDescriptors(), _filterStatus);
    }
    ...
    /// <summary>
    /// Renders the toolbar's filter status: quiet guidance toward the filter portal when nothing is
    /// applied, and the active filter plus how to clear it when one is.
    /// </summary>
    /// <returns>The markup for the status label.</returns>
    string FilterStatusMarkup()
    {
        var muted = Theme.Muted.ToMarkup();
        var key = Theme.Accent.ToMarkup();

        // Both hints carry their shortcut in parentheses, matching the toolbar's action buttons
        // ("View definition (D)"), with the key itself accented against the muted label so it reads
        // as a shortcut rather than part of the text. The Filter hint stays visible while a filter is
        // applied so the portal is always one click away — the applied text and Clear sit beside it.
        var filterHint = $"[link={FilterOpenLink}][{muted}]Filter ([/][bold {key}]F[/][{muted}])[/][/]";
        var clearHint = $"[link={FilterClearLink}][{muted}]Clear ([/][bold {key}]Esc[/][{muted}])[/][/]";

        return string.IsNullOrEmpty(_currentFilter)
            ? filterHint
            : $"{filterHint}  [{muted}]·[/]  [bold]{Markup.Escape(_currentFilter)}[/]  {clearHint}";
    }

    void SetFilterInput(string value)
    {
        if (_filterStatus is null)
        {
            return;
        }
    ...
    void DisposeControls()
    {
        ...
        // _filterStatus is owned by _toolbar (added as a toolbar item) — ToolbarControl.OnDisposing
        // disposes all its items, so we must NOT dispose _filterStatus here to avoid double-dispose.
    ...
    void OnTableRightClick(object? sender, MouseEventArgs e)
    {
        ...
        OnOpenContextMenu?.Invoke(e.AbsolutePosition.X, e.AbsolutePosition.Y, actions);
    }
    ...
    void RebuildRows()
    {
        ...
        // Checked rows survive the rebuild. ClearRows drops the check state with the rows, so a
        // refresh would silently empty a multi-selection the user had built up — and since the
        // rebuild now runs on every tick, a slow interval made that look random.
        var checkedKeys = HasCheckboxMode
            ? CheckedItems.Select(GetKey).ToHashSet(StringComparer.Ordinal)
            : [];
        ...
        {
            var row = new UITableRow(BuildRow(item)) { Tag = item };
            if (checkedKeys.Count > 0 && checkedKeys.Contains(GetKey(item)))
            {
                row.IsChecked = true;
            }
