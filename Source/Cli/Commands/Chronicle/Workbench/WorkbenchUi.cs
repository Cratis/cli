...
public static class WorkbenchUi
{
    ...
    /// <remarks>
    /// The background is set to an explicitly transparent color rather than left unset: an unset
    /// background makes the table resolve its container's solid color, painting a flat panel over
    /// the window gradient. Alpha-zero lets what is behind show through.
    /// </remarks>
    ...
    public static TableControlBuilder StyleDataTable(TableControlBuilder builder, ColorRole accentRole, SColor? separator = null) =>
        builder.WithSorting().NoBorder().WithColorRole(accentRole)
               .WithBackgroundColor(SColor.Transparent)
        ...
        DialogOptions? options = null)
    {
        ...
        var built = new WindowBuilder(windowSystem)
            ...
            // Flat, not gradient: over the few rows a dialog occupies the ramp reads as banding
            // rather than depth, and it fought the table hosted inside it.
