...
static class ReadModelJsonCleaner
{
    ...
    /// <c language="csharp">__initialized</c> field, and renames <c language="csharp">__lastHandledEventSequenceNumber</c> to
    /// <c language="csharp">LastHandledEvent</c>.
    /// </summary>
    ...
    /// <summary>
    /// Unwraps a <see cref="JsonNode"/> that may be a <c language="csharp">ConceptAs&lt;T&gt;</c> wrapper object
    /// serialized without converters (e.g. <c language="json">{"Value":42}</c> or empty <c language="csharp">{}</c>).
    /// Returns the inner <c language="csharp">Value</c> node when present, <see langword="null"/> when the object
