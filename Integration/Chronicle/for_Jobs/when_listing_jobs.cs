...
using context = Cratis.Cli.Integration.Chronicle.for_Jobs.when_listing_jobs.context;

namespace Cratis.Cli.Integration.Chronicle.for_Jobs;

[Collection(ChronicleCollection.Name)]
public class when_listing_jobs(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        public CliCommandResult Result = null!;

        async Task Because() => Result = await RunCliAsync("chronicle", "jobs", "list", "--event-store", "system");
    }
