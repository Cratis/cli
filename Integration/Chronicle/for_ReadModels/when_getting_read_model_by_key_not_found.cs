...
using context = Cratis.Cli.Integration.Chronicle.for_ReadModels.when_getting_read_model_by_key_not_found.context;
...
public class when_getting_read_model_by_key_not_found(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("chronicle", "read-models", "by-key", "nonexistent-read-model", "nonexistent-key", "--event-store", "system");
    }
