...
using Cratis.Cli;
using context = Cratis.Integration.Chronicle.Cli.for_EventTypes.when_showing_event_type.context;

namespace Cratis.Integration.Chronicle.Cli.for_EventTypes;
...
public class when_showing_event_type(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because()
        {
            var listResult = await RunCliAsync("chronicle", "event-types", "list", "--event-store", "system");
            ...
            Result = await RunCliAsync("chronicle", "event-types", "show", $"{EventTypeId}+{generation}", "--event-store", "system");
        }
    }
